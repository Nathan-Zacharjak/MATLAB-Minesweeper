...
function [grid] = mineGrid(rows, columns, percent, firstRow, firstColumn)
    ...
    while mineCount > 0
        ...
        % Some spaces may already have a mine, or be the first chosen
        % space, so skip it and try again
        if grid(row, column) ~= "*" ...
                && (allMines || row ~= firstRow && column ~= firstColumn)
            grid(row, column) = "*";
